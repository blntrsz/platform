...
deploy_issues_dev:
	export STAGE=dev && pnpm cdk deploy issues-app-dev --require-approval never

deploy_users_dev:
	export STAGE=dev && pnpm cdk deploy users-app-dev --require-approval never

deploy_host_dev:
	export STAGE=dev && export VITE_ISSUES_SITE=https://$(aws cloudformation describe-stacks --stack-name issues-app-dev --query 'Stacks[0].Outputs[?ExportName==`frontendUrl-issues-dev`].OutputValue' --output text) &&\
